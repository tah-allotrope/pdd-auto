...
class ProjectLocation(BaseModel):
    ...
    climate_zone: str | None = Field(
        None,
        description="IPCC climate zone; one of boreal_temperate_dry, boreal_temperate_wet, tropical_dry, tropical_wet",
    )
...
class AuxiliaryFuel(BaseModel):
    fuel_type: str = Field(..., description="Fuel type key matching constants.FOSSIL_FUEL_EF")

    annual_tonnes: float = Field(..., ge=0, description="Annual consumption tonnes/year")

    ncv_gj_per_tonne: float | None = Field(None, gt=0, description="NCV GJ/tonne override")

    ef_tco2_per_gj: float | None = Field(None, gt=0, description="EF tCO2/GJ override")

    source: str = Field(..., description="Source of consumption figure")


class RunoffWastewater(BaseModel):
    annual_volume_m3: float = Field(..., ge=0, description="Run-off wastewater volume m3/year")

    cod_t_per_m3: float = Field(..., ge=0, description="COD tCOD/m3")

    bo_t_ch4_per_t_cod: float = Field(0.25, gt=0, description="B_o tCH4/tCOD")

    mcf: float = Field(0.8, ge=0, le=1, description="Wastewater MCF")

    source: str = Field(..., description="Source of wastewater figures")
...
class ProjectTechnology(BaseModel):
    ...
    runoff_wastewater: RunoffWastewater | None = Field(
        None, description="Run-off wastewater for ACM0022 PE_WW (Eq.28)"
    )
...
class QuantificationInputs(BaseModel):
    ...
    grid_tdl_factor: float | None = Field(
        None, ge=0, le=0.3, description="Grid TDL loss factor 0-0.3"
    )
