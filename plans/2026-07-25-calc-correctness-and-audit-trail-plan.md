...
date: "2026-07-25"
status: "abandoned — closed by explicit user request on 2026-08-05 to clear the backlog and start fresh, after the user was asked to authorize the real-model proof's ~$12-16 spend and declined to proceed at this time. Re-verified 2026-08-13: PHASE-01 through PHASE-05 landed (811953c, a6b40da, 19d96b8, 02145a7, d08cfe8) and the code is genuinely in the tree (`AnnualErEntry`/`annual_schedule` and `swds_diversion_fraction` in calc/dispatch.py, `_add_calc_audit_appendix` + `emissions_summary`/`monitoring_tracked_params` renderers in export/docx_export.py, `PDD_CALC_AUTHORITATIVE` and `_build_calc_structured_content` in section_orchestrator.py), all PHASE-01-05 TASK boxes ticked. PHASE-06 (the real-model proof against `claude-code`) was never executed — `reports/` contains no `provider-scorecard-socson.md`, no `provider-scorecard-rice.md`, and no `2026-07-25-first-real-model-proof-findings.md`. Sibling plans 2026-07-16 and 2026-07-23 were abandoned on the same 2026-08-05 decision. If the proof run is wanted later, re-plan it fresh rather than resuming this file."
