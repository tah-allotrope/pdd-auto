...
def main() -> int:
    ...
    review_path = pipeline_run_path.with_suffix("").parent / (
        pipeline_run_path.stem + "-review.json"
    )
    review_data = None
    ...
    # Count metrics
    pipeline_stats = (
        count_tables_in_docx(pipeline_docx)
        if pipeline_docx.exists()
        else {"tables": 0, "paragraphs": 0, "pages_approx": 0}
    )
    ...
    sections_with_provenance = sum(
        1 for s in pipeline_data.get("sections", []) if s.get("provenance")
    )

    # Review layers
    tbd_count = review_data.get("tbd", {}).get("count", 0) if review_data else 0
    consistency_flags = (
        review_data.get("consistency", {}).get("critical_count", 0)
        + review_data.get("consistency", {}).get("high_count", 0)
        if review_data
        else 0
    )
